...
const AdminTimecard = ({ baseURL }: {baseURL: String}) => {
      // setting vars
...
async function A(PP: string) {
  await Http.post(`${baseURL}/admin/timecardsinterns`, {
    PP: PP,
  })
...
async function getinterntimecard(PP: string, intern:string) {
    if (intern){
    await Http.post(`${baseURL}/admin/gettimecard`, {
      PP: PP,
      intern:intern,
    })
    .then(function (response) {  
...
async function update_PP() {
  setloading("Loading")
  await Http.post(`${baseURL}/admin/update_timecard`, {
    Weeks: [Week1, Week2],
    PP: PP,
